...
def transfer(_to: address, _value: uint256) -> bool:
    ...
    if _value > 0:
        rewards.report(asset, msg.sender, _to, _value, 0) # @audit-ok call to rewards contract
...
def transferFrom(_from: address, _to: address, _value: uint256) -> bool:
    ...
    if _value > 0:
        ...
        rewards.report(asset, _from, _to, _value, 0) # @audit-ok call to rewards contract
...
def _deposit(_assets: uint256, _receiver: address):
    ...
    rewards.report(asset, empty(address), _receiver, _assets, pending) # @audit-ok call to rewards contract
...
def _withdraw(_assets: uint256, _receiver: address, _owner: address):
    ...
    pending: uint256 = self._pending()
    rewards.report(asset, _owner, empty(address), _assets, pending) # @audit-ok call to rewards contract
